...
class PollCreate(BaseModel):
    ...
    def validate_options(cls, v:List[str]) -> List[str]:
        if len(v) < 2 or len(v) > 5:
             raise HTTPException(
                status_code=400,
                detail="A poll options must be between 2 and 5"
             )
        return v
    ...
    def create_poll(self) -> "Poll":
        ...
        return Poll(title=self.title, options=choices,expires_at=self.expires )

class Poll(PollCreate):
    """Poll read data model, with uuid and createion date"""
    id: UUID = Field(default_factor=uuid4)
    options: List[Choice]
