...
# KV_REST_API_ULR = "url"
# KV_REST_API_TOKEN = "ASDKSAODOSAKDASOKD"
...
def get_all_polls() -> List[Poll]:
    ...
    polls = [
        Poll.model_validate_json(pj) for pj in poll_jsons if pj
    ]
    return polls
...
def get_vote_count(poll_id: UUID) -> Dict[UUID,int]:
    vote_counts = redis_client.ghetall("votes_count:{poll_id}")
...
def detel_poll(poll_id:UUID) -> None:
